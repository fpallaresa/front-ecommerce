...
const BreadcrumbsCategory = ({ categoryData }: BreadcrumbsCategoryProps): JSX.Element => {
  ...
  const parentCategory = !category?.parentCategory ? null : category?.parentCategory;

  const parentCategoryName = parentCategory && typeof parentCategory === "object" && "name" in parentCategory ? parentCategory.name?.es : "";
